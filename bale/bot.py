...
class Bot:
    ...
    def __init__(self, token: str, **kwargs):
        self.token = token
        self.loop = kwargs.get("loop") if kwargs.get("loop") is not None else asyncio.get_event_loop()
        self.http = HTTPClient(loop=self.loop, connector=kwargs.get("connector"), token=token)
        self._user = self.http.get_bot()
    ...
    def delete_webhook(self) -> bool:
        ...
        """
        response, payload = self.http.delete_webhook()
        return payload["result"]

    def send_message(self, chat_id: str, text: str = None, components=None, reply_to_message_id: str = None) -> Message | None:
        ...
        response, payload = self.http.send_message(chat_id, text, components=components, reply_to_message_id=reply_to_message_id)
        ...
        # create a function for convert key_lists in message, components
            ...
            for i in prices:
                if type(i) is Price:
                    ...
                    key_list.append(i)
            prices = key_list
        response, payload = self.http.send_invoice(chat_id, title, description, provider_token, prices, photo_url, need_name, need_phone_number, need_email, need_shipping_address, is_flexible)
        return Message.from_dict(data=payload["result"], bot=self)

    def edit_message(self, chat_id: str, message_id: str, text: str, components=None) -> Message:
        ...
        response, payload = self.http.edit_message_text(chat_id, message_id, text, components)
        return payload
    ...
    def delete_message(self, chat_id: str, message_id: str) -> bool:
        ...
        Return:
            bool: if done "True" if not "False"
        """
        response, payload = self.http.delete_message(chat_id, message_id)
        return payload["result"]
    ...
    def get_chat(self, chat_id: str) -> Chat:
        ...
        """
        response, payload = self.http.get_chat(chat_id)
        return Chat.from_dict(payload["result"], bot=self)

    def get_chat_member(self, chat_id: str, user_id: str) -> "ChatMember":
        """
            Args:
            ...
            Raises:
                :class:`bale.Error`
        """
        response, payload = self.http.get_chat_member(chat_id=chat_id, member_id=user_id)
    ...
    def get_chat_members_count(self, chat_id: str) -> int:
        """
            ...
            Raises:
                :class:`bale.Error`
        """
        response, payload = self.http.get_chat_members_count(chat_id)
        return payload["result"]

    def get_chat_administrators(self, chat_id: str) -> list["ChatMember"]:
        ...
        """
        response, payload = self.http.get_chat_administrators(chat_id)
        members = []
        for member_payload in payload["result"]:
            members.append(ChatMember.from_dict(data=member_payload))
        return members

    def get_updates(self, offset: int = None, limit: int = None) -> list["Update"] | None:
        ...
        """
        response, payload = self.http.get_updates(offset, limit)
        updates = []
